 ...
 * 
 * I made changes to this Class so that it will draw correctly a cannon or an explosion, depending on whether the user destroyed
 * the cannon by shooting straight up.
 */
			...
			while (threadIsRunning) {
				...
				try {
					...
					if (canvas != null) {
						...
						/**
						 * If the cannon is not hit, draw a normal cannon
						 */
						if (!Cannon.isHit) {
						...
						/**
						 * If the cannon is hit by the ball, as determined in Cannon class, draw an explosion instead
						 */
						else{

							Paint p = new Paint();
							Bitmap b = BitmapFactory.decodeResource(getResources(), R.mipmap.explosion);
							p.setColor(Color.BLACK);
							canvas.drawBitmap(b, 100, 1100, p);
						}
